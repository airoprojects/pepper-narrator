...
async function fetchData() {
    try {
        const response = await fetch('http://192.168.1.67:5000/request_data');  // Flask server URL
        const data = await response.json();
        console.log(data);  // Logging the entire dictionary
        updatePage(data);
    } catch (error) {
...
function sendInteger(vote_id) {
    fetch('http://192.168.1.67:5000/submit_integer', {  // Flask server URL
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
const eventSource = new EventSource('http://192.168.1.67:5000/stream');  // Flask server URL
